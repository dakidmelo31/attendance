        ...
        body: !cameraController.value.isInitialized
          ...
          children: [
            ...
            Card(
              ...
              margin:const EdgeInsets.symmetric(horizontal: 25.0, vertical: 4),
              child: TextField(
                decoration: InputDecoration(
                  hintText: "Your Name",
                    ...
                    focusedErrorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15.0)),
                    filled: false
                ),
              ),
            ),
...
List<Face> facesDetected = []
  _predictFacesFromImage({required CameraImage image}) async{
    await Future<viod> detectFacesFromImage(image);
  ...
  Future<void> detectFacesFromImage(CameraImage image) async {
    InputImageData _firebaseImageMetadata = InputImageData(size: Size(image.width.toDouble(), image.height.toDouble()), imageRotation: rotationIntToImageRotation(cameraController.description.sensorOrientation), inputImageFormat: InputImageFormat.bgra8888, planeData: image.planes.map((e) => InputImagePlaneMetadata(bytesPerRow: e.bytesPerRow,height: e.height, width: e.width)).toList());
  InputImage _firebaseVisionImage = InputImage.fromBytes(bytes: image.planes[0].bytes, inputImageData: _firebaseImageMetadata);
  var result = await _googleMlKit.faceDetector().processImage(_firebaseVisionImage);
